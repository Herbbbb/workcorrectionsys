...
<BODY>
<form action="/workcorrectionsys/sys.do?action=uppf" name="f1" method="post"> 
...
border=0>
  ...
  String s = "select * from mykc where id ="+id;
  HashMap map = dao.select(s).get(0);
   %>
  <TR>
  <input type="hidden" id="id" name="id" value="<%=id %>" >
    <TD width="35%" height=24 align="center" class=forumrow>分数</TD>
    <TD class=forumrowhighlight width="65%" 
      height=24><label>
      <input name="score" type="text" id="score" value="<%=map.get("score") %>" >
    </label></TD></TR>
...
if(suc!=null)
{
 %>
 alert("操作成功");
  parent.location.replace("/workcorrectionsys/pages/mystudents.jsp");
 <%}%>
 </script>
