...
HashMap m = dao.select("select * from myzy where id="+myzyid+"").get(0);
%>
<form action="/workcorrectionsys/sys.do?action=zypf&myzyid=<%=myzyid %>" name="f1" method="post" onsubmit="return ck()" > 
<TABLE cellSpacing=1 cellPadding=3 width="100%" align=center bgColor=#6ab6b6 
border=0>
  <TBODY>
 
  <TR>
    <TD width="20%" height=24 align="center" class=forumrow>分数</TD>
    <TD class=forumrowhighlight 
      height=24><label>
      <input name="score" type="text" id="score" value="<%=m.get("score") %>">
    </label></TD></TR>
...
if(suc!=null)
{
 %>
 alert("操作成功");
  parent.location.replace("/workcorrectionsys/pages/zygl.jsp");
 <%}%>
 </script>
