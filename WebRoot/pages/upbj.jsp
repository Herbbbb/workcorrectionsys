...
<BODY>
  ...
  String s = "select * from bj where id ="+id;
  HashMap map = dao.select(s).get(0);
   %>
<form action="/workcorrectionsys/sys.do?action=upbj" name="f1" method="post" onsubmit="return ck()"> 
...
border=0>
  ...
  <TR>
    <TD width="35%" height=24 align="center" class=forumrow>请输入班级名</TD>
    <TD class=forumrowhighlight width="65%" 
      height=24><label>
      <input name="bjname" type="text" id="bjname">
    </label></TD></TR>
  <TR>
...
if(suc!=null)
{
 %>
 alert("操作成功");
  parent.location.replace("/workcorrectionsys/pages/bj.jsp");
 <%}%>
 </script>
<%=Info.tform(map)%>