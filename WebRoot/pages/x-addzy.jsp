...
<BODY>
<form action="/workcorrectionsys/sys.do?action=addzy" name="f1" method="post" onsubmit="return ck()" enctype="multipart/form-data"> 
            <%HashMap admin = (HashMap)request.getSession().getAttribute("admin"); 
            CommDAO dao = new CommDAO();%>
            <input id="teacherid" name="teacherid" type="hidden" value="<%=admin.get("id") %>">
...
border=0>
 ...
 <TR>
    ...
    <TD class=forumrowhighlight width="85%" 
      ...
      for(HashMap m:list){%>
      	<option value="<%=m.get("id") %>"><%=m.get("kcname") %></option>
      	<%} %>
      </select>*
    </label></TD></TR>
  <TR>
    <TD  height=24 align="center" class=forumrow>标题</TD>
    <TD class=forumrowhighlight width="85%" 
      height=24><label>
    ...
    <TD class=forumrowhighlight width="85%" 
     ...
     <TR>
    <TD  height=24 align="center" class=forumrow>说明</TD>
    <TD class=forumrowhighlight width="85%" 
      height=24><label>
